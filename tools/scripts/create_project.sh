...
export DESTINATION="$(realpath ${DESTINATION})/${NAME}s"
...
if [ "$is_create" -eq "1" ]; then
  cd ${SOURCE}/firmware
  ## make can only be run from a container, where Vivado environment is likely not set/inexistent
  ##make
  cd $curwordir
  mkdir -p logs
...
if [ "$is_program" -eq "1" ]; then
  bitstream_file="${DESTINATION}/${NAME}.runs/impl_1/system.bit"
